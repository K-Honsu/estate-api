...
export class AuthService {
  ...
  ): Promise<{ message: string; data: RegisterBusinessResponseDto }> {
    try {
      ...
      if (existingCompany) {
        throw new ConflictException(
          'Company registration number already in use',
        );
      }
  ...
  async validateUser(
    ...
    try {
      ...
      this.logger.error('Error:', error.response?.data || error.message);
      throw new BadRequestException('Failed to validate user', error.message);
    }
  }
